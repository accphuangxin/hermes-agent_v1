...
DEFAULT_CONFIG = {
    ...
    # Gateway platform configurations (written to config.yaml under "platforms:")
    # Each entry maps directly to gateway/config.py PlatformConfig.from_dict().
    "platforms": {
        "api_server": {
            "cors_origins": "*",
            "enabled": True,
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # ── Version 23 → 24: seed default platforms.api_server config ──
    if current_ver < 24:
        config = read_raw_config()
        raw_platforms = config.get("platforms")
        if not isinstance(raw_platforms, dict):
            raw_platforms = {}
        if "api_server" not in raw_platforms:
            default_api_server = copy.deepcopy(
                DEFAULT_CONFIG.get("platforms", {}).get("api_server", {})
            )
            raw_platforms["api_server"] = default_api_server
            config["platforms"] = raw_platforms
            save_config(config)
            results["config_added"].append("platforms.api_server")
            if not quiet:
                print("  ✓ Added platforms.api_server defaults to config.yaml")
